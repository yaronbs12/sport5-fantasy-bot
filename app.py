...
<button onclick="(function(btn){{var txt=document.getElementById('report-text-{i}').value;var orig=btn.innerHTML;var ta=document.createElement('textarea');ta.value=txt;ta.style.position='absolute';ta.style.left='-9999px';document.body.appendChild(ta);ta.select();try{{if(document.execCommand('copy')){{btn.innerHTML='<span>✓</span> הועתק!';btn.style.backgroundColor='#10b981';setTimeout(function(){{btn.innerHTML=orig;btn.style.backgroundColor='#ef4444';}},2000);}}else{{window.prompt('העתק את הדוח:',txt);}}}}catch(e){{window.prompt('העתק את הדוח:',txt);}}document.body.removeChild(ta);}})(this)" style="flex: 1; height: 42px; background-color: #ef4444; color: white; border: none; border-radius: 8px; cursor: pointer; font-weight: 600; box-shadow: 0 4px 10px rgba(239, 68, 68, 0.25); display: inline-flex; align-items: center; justify-content: center; font-size: 14.5px; font-family: 'Rubik', sans-serif;">📋 העתק דוח ללוח</button>
</div>"""
